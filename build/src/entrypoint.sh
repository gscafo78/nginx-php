...
if [ ! -d /var/www/html/conf/virtualhost ]; then
    mkdir -p /var/www/html/conf/virtualhost
fi
...
if [ "$(ls -A /var/www/html/conf/virtualhost/*.conf 2>/dev/null)" ]; then
  info "Custom nginx sites configs found"
  rm /etc/nginx/conf.d/*
  cp /var/www/html/conf/virtualhost/*.conf /etc/nginx/conf.d/
  info "Start nginx with custom server configs..."
else
  info "nginx-site.conf not found"
  info "If you want to use custom configs, create config file in /var/www/html/conf/virtualhost/nginx-site.conf"
